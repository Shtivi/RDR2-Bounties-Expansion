...
Ped BuckHicoxExecutor::spawnTarget()
{
	Vector3 targetPos = toVector3(-2116.705, 351.9135, 136.5443);
	Ped target = createPed("G_M_M_UniRanchers_01", targetPos);
	return target;
}

void BuckHicoxExecutor::enterIdleMode()
{
	char* scenarioName;
	Ped player = PLAYER::PLAYER_PED_ID();
	vector<Ped>::iterator pedItr;
...
void BuckHicoxExecutor::enterAlertMode()
{
	vector<Ped>::iterator pedItr;
	for (pedItr = enemies.begin(); pedItr != enemies.end(); pedItr++)
	{
...
void BuckHicoxExecutor::enterWarningMode()
{
	vector<Ped>::iterator pedItr;
	for (pedItr = enemies.begin(); pedItr != enemies.end(); pedItr++)
	{
...
void BuckHicoxExecutor::enterCombatMode()
{
...
void BuckHicoxExecutor::prepareSet()
{
	...
	this->horse = createPed("A_C_Horse_Turkoman_Gold", toVector3(-2126.751, 366.3427, 134.626));
	addHorse(horse);
	addHorse("A_C_Horse_KentuckySaddle_Black", toVector3(-2125.183, 369.4261, 133.4568));
	addHorse("A_C_Horse_KentuckySaddle_SilverBay", toVector3(-2123.578, 372.5853, 132.3413));

	addEnemy(target);
	addEnemy(toVector3(-2126.761, 354.9373, 138.4859));
	addEnemy(toVector3(-2124.129, 356.6917, 137.1421));
	addEnemy(toVector3(-2131.327, 360.7215, 137.2773));
	addEnemy(toVector3(-2123.272, 351.4809, 138.4948));
	addEnemy(toVector3(-2106.465, 361.7401, 131.6631));
	addEnemy(toVector3(-2133.81, 371.7192, 134.1191));
	addEnemy(toVector3(-2121.923, 344.3732, 148.5946));
...
void BuckHicoxExecutor::createEnemyBlips()
{
	std::vector<Ped>::iterator it;
...
void BuckHicoxExecutor::addEnemy(Vector3 pos)
{
	Ped enemyPed = createPed("G_M_M_UniRanchers_01", pos);
	addEnemy(enemyPed);
}

void BuckHicoxExecutor::addEnemy(Ped ped)
{
